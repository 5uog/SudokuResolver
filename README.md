...
数独という一見単純な数的パズルは、数学的には有限領域 <ruby>制約充足問題<rt>せいやくじゅうそくもんだい</rt></ruby> (Constraint Satisfaction Problem, CSP) の典型例として記述することができ、その厳密な数理定式化を通じて、パズルの領域を超えた学術的な意味を帯びることになる。一般に CSP は三つ組 $(V,D,C)$ によって定義される。すなわち $V$ は変数集合、 $D$ は各変数の取り得る領域 (domain)、 $C$ は制約集合である。数独の場合、変数集合は $`9 \times 9`$ の格子上のセルに対応し、

$$V = \{ x_{r,c} \mid r,c \in \{1,\dots,9\} \}, \qquad D = \{1,\dots,9\}$$

と定義できる。各 $x_{r,c}$ はセル $(r,c)$ に置かれる数字を表す。制約集合 $C$ は「各行・各列・各 $3 \times 3$ ブロックにおいて 1 から 9 の数字が重複しない」という三種類の **<ruby>all-different 制約<rt>オールディファレントせいやく</rt></ruby>** に集約され、形式的には次のように記述される。
...
ここで $B$ は $3 \times 3$ ブロックのいずれかである。さらに問題文として与えられる与え値 (givens) は、特定セルに対する固定制約 $x_{r,c}=d$ ($d \in \{1,\dots,9\}$) として追加される。したがって数独とは「与えられた初期制約を含む CSP を解く問題」であると定義できる。

この枠組みに基づけば、盤面 $P$ が<ruby>充足可能<rt>じゅうそくかのう</rt></ruby> (satisfiable) であるとは、次の述語が真となることである。

$$
\mathsf{SAT}(P) := \exists \, \sigma: V \to \{1,\dots,9\} \ \text{s.t. } \sigma \text{ はすべての制約を満たす}
$$

すなわち全てのセルに数字を割り当てる写像 $\sigma$ が存在し、かつその割り当てが制約集合 $C$ をすべて満たすとき、盤面は解を持つ。一方で<ruby>一意性<rt>いちいせい</rt></ruby> (uniqueness) は次のように定義される。

$$
U(P) := \exists! \, \sigma: V \to \{1,\dots,9\} \ \text{s.t. } \sigma \text{ はすべての制約を満たす}
$$
...
より一般化された $n^2 \times n^2$ 格子上の数独は、充足可能性判定が NP 完全であることが知られている（Yato–Seta, 2003）。探索空間は指数関数的に拡大し、 $9 \times 9$ の場合でも盤面全体の配置は $9^{81} \approx 10^{77}$ 通りに及ぶ。さらに Felgenhauer–Jarvis (2006) によれば、完成盤は $6.67 \times 10^{21}$ 通り存在し、その中から一意解を持つ良問を抽出することは決して自明ではない。この桁外れの数的基盤が、数独を NP 完全問題の一例としながらも、人間が現実的時間内に解けるという事実と強い対照をなしている。
...
$$
\mathrm{Cost}_{\mathrm{search}} = \lambda \cdot N_{\mathrm{branch}} + \mu \cdot \sum_{\mathrm{branch}} \mathrm{depth}
$$

の形で定義できる。ここで $\lambda,\mu$ は探索の導入を強く罰するための重みである。
...
$$
\mathrm{Diff} = \alpha \cdot \mathrm{Cost}_{\mathrm{logic}} + \beta \cdot \mathrm{Cost}_{\mathrm{search}} - \gamma \cdot \sum_t \Delta H_t
$$

で与えられる。ここで $\alpha,\beta,\gamma$ は実験的に決定される重みであり、既存の問題集に対して回帰的に最適化される。こうして得られた $\mathrm{Diff}$ は、問題を難易度クラス（Easy, Medium, Hard, Expert など）に分類する客観的な基準として機能し、ユーザが問題を選択する際の有効な指標となる。
...
p = \min\left\{\,1,\;
    \exp\!\left(
        -\frac{\lvert \mathrm{Diff}-\mathrm{Diff}^* \rvert
            - \lvert \mathrm{Prev}-\mathrm{Diff}^* \rvert}{T}
    \right)
\right\}
$$

によって与えられる。ここで $T$ は温度パラメータであり、反復が進むにつれて徐々に低下させることで局所最適からの脱出と大域的収束を両立させる。

この手続きを繰り返すことで、最終的に得られる問題は常に一意解性を保ちながら、所望の難易度に漸近する。したがって本方式によって生成される数独問題は、単に解を持つだけでなく、難易度が設計者の意図に沿った水準に調整されている。アプリケーションの利用者はこれにより、容易な問題から高度に難解な問題まで、自身の学習目的や娯楽的要求に応じて問題を選択・生成することが可能となる。
...
まず最も基礎的な水準において、選択されたセルを基準にその行・列・ボックスに属するセルを同系統の彩色で強調することが求められる。この操作は形式的には all-different 制約の視覚的写像に相当する。すなわち集合 $\{\,x_{r,j}\,\}_{j=1}^9$ , $\{\,x_{i,c}\,\}_{i=1}^9$ , さらに $3\times 3$ ブロック内の変数群がすべて相異なる値を取らなければならないという制約を、抽象的な論理式から色彩空間への埋め込みによって直観的に把握可能とするのである。これは単に操作性を補助するのみならず、制約充足問題における局所的一貫性を可視的に体現するものにほかならない。
