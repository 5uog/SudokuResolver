...
$$
\sigma^*(r,c) \in \sigma(r,c) \ \implies \ \sigma^*(r,c) \in \gamma(\sigma)(r,c)
$$
...
$$
\mathrm{Cost}_{\text{search}} = \lambda \cdot N_{\text{branch}} + \mu \cdot \sum_{\text{branch}} \mathrm{depth}
$$
...
$$
\mathrm{Diff} = \alpha \cdot \mathrm{Cost}_{\text{logic}} + \beta \cdot \mathrm{Cost}_{\text{search}} - \gamma \cdot \sum_t \Delta H_t
$$
...
$$
p = \min\{1, \exp(-( |\mathrm{Diff}-\mathrm{Diff}^*| - |\mathrm{Prev}-\mathrm{Diff}^*| )/T )\}
$$
